				...
				if(result.getObj() != null) {
					...
					if(before > 0 ) {
						if(udList != null && udList.size() > 0 && uList != null && uList.size() > 0) {
							...
							Integer size = uList.size();
							if (pointsDataLimit < size) {// 判断是否有必要分批
								int part = size / pointsDataLimit;// 分批数
								for (int i = 0; i < part; i++) {
									List<UserDepartmentEntity> udListPage = udList.subList(0, pointsDataLimit);
									List<UsersEntity> uListPage = uList.subList(0, pointsDataLimit);
									userDepartmentService.saveBatch(udListPage);
									usersService.saveBatch(uListPage);
									udList.subList(0, pointsDataLimit).clear();
									uList.subList(0, pointsDataLimit).clear();
								}
								if (!udList.isEmpty() && !uList.isEmpty()) {
									userDepartmentService.saveBatch(udList);
									usersService.saveBatch(uList);
								}
							} else {
								userDepartmentService.saveBatch(udList);
								usersService.saveBatch(uList);
						...
						if(udList != null && udList.size() > 0 && uList != null && uList.size() > 0) {
							...
							if (pointsDataLimit < size) {// 判断是否有必要分批
								...
								for (int i = 0; i < part; i++) {
									List<UserDepartmentEntity> udListPage = udList.subList(0, pointsDataLimit);
									List<UsersEntity> uListPage = uList.subList(0, pointsDataLimit);
									userDepartmentService.firstSaveBatch(udListPage);
									usersService.firstSaveBatch(uListPage);
									udList.subList(0, pointsDataLimit).clear();
									uList.subList(0, pointsDataLimit).clear();
								}
								if (!udList.isEmpty() && !uList.isEmpty()) {
									userDepartmentService.firstSaveBatch(udList);
									usersService.firstSaveBatch(uList);
								}
							} else {
								userDepartmentService.firstSaveBatch(udList);
								usersService.firstSaveBatch(uList);
